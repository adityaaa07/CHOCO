...
import NowPlaying from '../Components/NowPlaying'; // << NEW
...
const Homepage = () => {
  ...
  useEffect(() => {
    ...
    return () => unsub();
  }, [isLeaving]); // Refresh when leaving

  return (
    <>
      <Sidebar />
      <div className="flex flex-col bg-black min-h-screen">
        {/* NowPlaying bar at the top */}
        <NowPlaying />

        <div className="flex flex-wrap justify-center gap-2 w-screen" id="top">
          <CreateRoom />
          <JoinRoom />
            ...
            {Cookies.get('name') &&
              ...
              pathName.includes('home') && (
                <div className="text-white mt-3 text-lg ml-3 flex items-center">
                  <b>Welcome {Cookies.get('name').split(' ')[0] || Cookies.get('name')}</b>
                </div>
              )}
            ...
            {sessionStorage.getItem('roomCode') &&
              currentSong.length > 0 && (
                <div className="flex items-center justify-center flex-col">
                  ...
                  {/* No need for YouTubeVideo here. NowPlaying handles player! */}
                </div>
              )}
          </div>
        </div>
