...
function CreateRoom() {
  ...
  const handleCreateRoom = async () => {
    ...
    // Use Spotify user ID or fallback to 'Guest' if not available
    const userName = Cookies.get('name') || sessionStorage.getItem('spotify_user_id') || 'Guest';
    try {
      ...
      // Set user name in cookies for consistency
      ...
      setMsg('Error creating room. Try a different code.');
  ...
  return (
    <Modal centered={true} className="flex justify-center w-72" isOpen={modal_backdrop} toggle={toggle}>
      <ModalHeader toggle={toggle}>
        <b>Create a new room</b>
      </ModalHeader>
      <ModalBody>
        {msg && <h6 className="m-3 text-start"><b>Here's the link</b></h6>}
