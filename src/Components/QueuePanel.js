...
const QueuePanel = () => {
  ...
  useEffect(() => {
    const initializeRoom = async () => {
      try {
        ...
        console.log('QueuePanel: Room document initialized:', roomCode);
      } catch (error) {
        console.error('QueuePanel: Error initializing room document:', error);
      }
    };
    ...
    const unsubscribe = onSnapshot(docRef, snapshot => {
      ...
      if (data) {
        ...
        console.warn('QueuePanel: No document found for room:', roomCode);
      }
    }, err => {
      console.error('QueuePanel: Firestore snapshot error:', err);
    });
  ...
  const handlePlayFromQueue = async track => {
    ...
    if (track.platform === 'spotify') {
      const sessionToken = sessionStorage.getItem('spotify_token');
      const isValidUri = track.uri && track.uri.startsWith('spotify:track:');
      console.log('QueuePanel: Spotify props:', {
        uri: track.uri,
        isValidUri,
        image: track.image,
        title: track.title,
      ...
      if (!isValidUri) {
        console.error('QueuePanel: Invalid Spotify URI format. Expected spotify:track:<id>');
      }
      if (!token && !sessionToken) {
        console.warn('QueuePanel: No Spotify token available. Redirecting to login...');
        navigate('/');
      }
    }
  };

  console.log('QueuePanel: Current playing track:', currentPlaying);
  if (currentPlaying?.platform === 'spotify' && !currentPlaying?.uri) {
    console.warn('QueuePanel: Spotify track missing URI:', currentPlaying);
  }
  ...
  return (
    <div className="bg-zinc-900 text-white p-4 rounded-lg overflow-y-auto max-h-64">
      ...
      <div id="player-container" className="mt-4">
        {currentPlaying?.platform === 'youtube' && <YouTubeVideo videoIds={[currentPlaying]} />}
        {currentPlaying?.platform === 'spotify' && currentPlaying?.uri && currentPlaying?.uri.startsWith('spotify:track:') && token && (
          <SpotifyPlayer
            uri={currentPlaying.uri}
            image={currentPlaying.image || ''}
            title={currentPlaying.title || ''}
